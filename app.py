...
def send_email_reminder(to_email, invoice):
    """Send real email reminder via Gmail SMTP"""
    try:
                ...
                <div style="background: white; padding: 20px; border-radius: 10px; margin: 20px 0;">
                    <table style="width: 100%;">
                        ...
                        <tr>
                            ...
                            <td style="color: #ef4444; font-weight: bold; font-size: 1.2em;">{format_currency(remaining)}</td>
                        </tr>
                    </table>
                </div>
                
                <p style="color: #ef4444;"><strong>⚠️ This invoice is {days_overdue} days overdue.</strong></p>
...
def extract_with_openai(file_bytes, file_type):
    """Extract using OpenAI API (GPT-4o)"""
    try:
            ...
            json={
                ...
                "messages": [{
                    ...
                    "content": [
                        ...
                        {
                            "type": "text",
                            "text": 'Extract invoice/contract data from this document. Return ONLY valid JSON: {"client_name":"","client_email":"","project_name":"","contract_value":0,"items":[{"description":"","qty":1,"price":0}],"payment_terms":"Net 30","due_date":"YYYY-MM-DD"}. Return ONLY JSON.'
                        }
                    ]
                }],
                "max_tokens": 1024
        ...
        if inv:
            ...
            with action_col1:
                if remaining > 0 and inv['status'] != 'draft':
                    if st.button("📧 Send Email Reminder", key=f"remind_{inv['id']}", use_container_width=True):
                        with st.spinner("Sending email..."):
                            success, message = send_email_reminder(inv['client_email'], inv)
                            if success:
...
# NEW INVOICE
    ...
    st.info("🤖 Upload a contract/invoice and AI will extract the data!")
        ...
        with col1:
            ...
            if 'pdf' in uploaded.type:
                ...
                try:
                    ...
                    st.warning("Could not preview PDF")
            else:
        ...
        with col2:
            ...
            if st.button("🚀 Extract with AI", type="primary", use_container_width=True):
                with st.spinner("🤖 AI is reading the document..."):
                    ...
                    result = extract_with_openai(file_bytes, uploaded.type)
                    if result:
                        st.session_state.extracted_data = result
                        st.success("✅ Data extracted!")
                    else:
                        st.error("❌ Failed to extract. Try a clearer document.")
...
# Footer
st.divider()
st.caption("InvoiceMatch ✨ Powered by OpenAI + Gmail")
